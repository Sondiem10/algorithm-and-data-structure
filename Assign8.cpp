...
void List_Add(ListSV* list, SinhVien SV) {
    ...
    if (list->first == nullptr) { // If the list is empty
        ...
        newnode->link = list->first;// Point new node to the old first
        list->first = newnode; // Update first to new node
    }
}
int List_Length(ListSV* list) {
    Node* node = list->first;
    int i = 0;
    while (node != nullptr) { // Check if the current node is not null
        i++;
...
void BubbleSort(ListSV* list) {
    if (list->first == nullptr) return; // Empty list, no need to sort
...
void PrintSameBirthday(ListSV* list) {
    ...
    while (current != nullptr) {
        ...
        cout << "Students with the same birthday as " << current->SV.hoTen << " ("
        ...
        if (!found) {
            cout << "No other students found with the same birthday.\n";
        }
